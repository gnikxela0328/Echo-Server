...
char *hash (char *username) {
	unsigned char *digest = SHA256(username, strlen(username), 0);
	char *hashResult = malloc(((SHA256_DIGEST_LENGTH) * 2) + 1);

	for (int i = 0; i < (SHA256_DIGEST_LENGTH); i++) {
		char temp[5];
		sprintf(temp, "%X", digest[i]);
		strcat(hashResult, temp);
	}
...
int main (int argc, char **argv) {
	...
	for ( ; ; ) {
		...
		if ((strcmp(buf, hashResult))) {
			printf("User authentication failed. Disconnecting... \n");
			exit(0);	
		}
